 ...
 * We follow its logic here to get the same working connection.
 *
 * <p>
 * This exists solely to forward input creation to WebView's
 * ThreadedInputConnectionProxyView on the IME thread. The way that this is
 * created in {@link InputAwareWebView#checkInputConnectionProxy} guarantees
 * that we have a handle to ThreadedInputConnectionProxyView and
 * {@link #onCreateInputConnection} is always called on the IME thread. We
 * delegate to ThreadedInputConnectionProxyView there to get WebView's input
 * connection.
 */
final class ThreadedInputConnectionProxyAdapterView extends View {
   ...
   * this in {@link InputAwareWebView#checkInputConnectionProxy(View)}.
   *
   * <p>
